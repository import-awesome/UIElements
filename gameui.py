...
class Menu(object):
	def __init__(self, screen, win_x, win_y, stats):
		self.screen = screen
		self.size_x = win_x
		self.size_y = win_y
...
class UI(object):
	...
	def Display(self):
		...
		for x in self.characters:
			...
			hp_t = self.small.render("HP: {} / {}".format(x.hp, x.hp_max), 1, WHITE)

			self.screen.blit(name_t, (self.char1_x + (shift * 4 * self.char1_x), self.char_y))
			self.screen.blit(hp_t, (self.char1_x - 25 + (shift * 4 * self.char1_x), self.char_y+20))
			shift += 1
...
if __name__ == "__main__":
	...
	display = True
	go = False
	c = chartemplate.Character(1, "TEST", 100, 10, [1, 1, 1, 1, 1, 1])
	c1 = chartemplate.Character(1, "TEST1", 100, 10, [1, 1, 1, 1, 1, 1])
	c2 = chartemplate.Character(1, "TEST2", 100, 10, [1, 1, 1, 1, 1, 1])
	c3 = chartemplate.Character(1, "TEST3", 100, 10, [1, 1, 1, 1, 1, 1])
	chars = [c, c1, c2, c3]
